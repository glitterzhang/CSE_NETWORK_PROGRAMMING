...
main(int argc, char const *argv[])
{
    ...
    //Constructing socket name of the troll to send to
  	...
  	troll.sin_port = htons(atoi(argv[1]));
	...
	while(1) {
		...
		//calculate checksum and save it into tcpheader
        ...
        memcpy(data, &message.tcpheader,rec-sizeof(message.header));// can't use &message+16
	   	...
	   	uint16_t checksum=crc16(0,data,rec-sizeof(message.header));
        message.tcpheader.check=htons(checksum);
        printf("%s\n","Sent checksum");
        printf(" %u\n",checksum);
